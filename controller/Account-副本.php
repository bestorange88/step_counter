...
// 三方代付平台API配置
    ...
    public function submit(Request $request)
    {
        ...
        // 检查用户余额是否足够
        $account = Db::name('account')->where('uid', $uid)->find();
        if ($account['balance'] < $data['money']) {
            return json(['code' => 400, 'message' => '余额不足']);
        }

        // 冻结用户资金
        Db::name('account')->where('uid', $uid)->update([
            'balance' => $account['balance'] - $data['money'],
            'frozen' => $account['frozen'] + $data['money']
        ]);
        ...
        // 保存到数据库
        $result = Db::name('withdraw_requests')->insert($data);
        ...
        if ($result) {
            ...
            if ($response['code'] === 200) {
                ...
                return json(['code' => 500, 'message' => '提交失败：' . $response['message']]);
            }
        } else {
    ...
    private function initiateWithdraw($data)
    {
        $params = [
            ...
            'money' => $data['money'],
            'notifyurl' => 'http://yourdomain.com/api/withdraw/callback', // 设置你的回调地址
    ...
    {
        return uniqid('tx_'); // 使用uniqid生成唯一ID，前缀为tx_
    }
