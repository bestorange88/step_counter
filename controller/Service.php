...
class Service extends Base
{
    ...
    public function sms(Request $req)
    {
        ...
        // 调用接口
        if(array_key_exists('tecent', $config)){
            $result = $this->tecent_sms($mobile, $data[0], $config['tecent']);
        }
        /*
    ...
    public function tecent_sms($mobile, $captcha, $config){
        $templateId = 379301;
        try {
            $ssender = new \Qcloud\SmsSingleSender($config['appid'], $config['appkey']);
            $params = [$captcha];
            $result = $ssender->sendWithParam("86", $mobile, $templateId, $params, "", "", "");  // 签名参数未提供或者为空时，会使用默认签名发送短信
