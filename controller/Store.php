...
class Store extends Base
{
    ...
    public function buy(Request $req)
    {
      ...
      $wallet_money = Db::table('wallet')->where(['username'=>$username])->find();
       
        if($product['price']>$wallet_money['money']){
            return [
                'code'      =>  511,
                'message'   =>  '余额不足,请充值'
            ];
...
//            } else {  }
      ...
      //      $wallet_money = Db::table('wallet')->where(['username'=>$username])->find();
        ...
        //        if($product['power']>$wallet_money['money']){
          //          return [
            //            'code'      =>  403,
              //          'message'   =>  '挑战卡不足,请充值'
                //    ];
...
//                }
          ...
          if ($product['catalog'] == 2) {
                ...
                $_result = Db::table('punch_order')->insert($_data);
                $_res =  Db::table("dashboard")->where(array("username" => $username))->setDec('power',$_data['power']);
