...
class Event extends Base
{
 ...
 public function get_rand(array $proArr): ?string {
        ...
        foreach ($proArr as $key => $proCur) {
            ...
            $randNum = mt_rand(1, round($proSum * 1000000)); 
            $cumulative = round($proCur * 1000000);
 ...
 public function lottery(Request $req)
     ...
     * 大转盘
     */
    public function turntable(Request $req)
    {
        // 用户账号
        $username = session('user.account.username');
        // 得到所有的奖品
        $rewards = config('hello.event.truntable.reward');
        // 是否开启
        if (empty(config('hello.event.truntable.enable'))) {
        ...
        if ($req->isPost()) {
            try {
                ...
                // 查询记录次数
                $count = Db::table('event_log')->where('username', '=', $username)->where('type', '=', self::TYPE_TURNTABLE)->count('id');
                // 查询机会
               // $user = getData('account', ['username' => $username], 1);
                $points=$user['account']['points'];
                $cishu = (int) floor($points / 50);
               // dump($cishu);
               // dump($count);
               // die;
                $myChance = config('hello.level.' . $user['account']['type'] . '.truntable') ?: 0;
                $chance = config('hello.event.truntable.chance');
                ...
                $myRewardLogCount = Db::table('event_log')
                    ...
                    ->where('type', '=', self::TYPE_TURNTABLE)
                    ->where('hit', '=', 1)
                    ->group('reward')
                    ->select();
                // 所有已中奖的数量
            ...
            return json([
                ...
                'data'      =>  [
                    ...
                    'reward'=>  [
                        ...
                        'deg'  =>  empty($reward) ? '0' : $reward['deg'],
                        'index'  =>  empty($reward) ? '0' : array_search($reward['name'], $rewards),
                        'date'  =>  date('Y-m-d'),
        ...
        $this->assign('myChance', $cishu + $myChance);
        // 查询我的中奖记录
