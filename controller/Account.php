   ...
   public function sendEmailCode($email)
    ...
    private function sendSmsCode($userPhone)
    {
        ...
        try {
            ...
            if ($result->Code == 'OK') {
                Session::set("phone_code_{$userPhone}", strval($smsCode));
...
public function edit(Request $req)
    {
       ...
       // 获取提交的数据
        $data = $req->only(['nickname', 'wechat', 'qq', 'alipay', 'bankname', 'bankcard', 'bankaddress']);
         ...
         // 修改账户
        $this->updateAccount($username, $data);

        // 保存日志
        $this->log('update_profile', 'User updated profile', $username);
     ...
     * @param array $data
     */
    protected function updateAccount($username, $data)
    {
        Db::table('account')->where('username', $username)->update($data);
    }
...
// 三方代付平台API配置
    ...
    public function submit(Request $request)
    {
        ...
        $data['bankcard'] = $params['cardnumber'];
...
// 定义错误消息
$msg = [
    'type.require'        => '注册类型不能为空！',
    'type.in'             => '无效的注册类型！',
    'target.require'      => '注册目标不能为空！',
    'password.require'    => '密码不能为空！',
    'password.length'     => '密码长度必须在 6 到 32 个字符之间！',
    'captcha.require'     => '验证码不能为空！',
    'captcha.number'      => '验证码必须是数字！',
    'captcha.length'      => '验证码长度必须为 6 位！',
];
...
$type = $req->param('type');
$target = $req->param('target');
$verifyCode = $req->param('captcha');

    $sessionKey = "{$type}_code_{$target}";

    $storedCode = Session::get($sessionKey);

  if (!$storedCode || strcmp($storedCode, (string)$verifyCode) !== 0) {
   // return json(['code' => 401, 'message' => '验证码不正确或已过期']);
  }
 ...
 //return json(['code' => 200, 'message' => '验证码验证成功']);
            ...
            try {
                ...
                Db::startTrans();
                 ...
                 $user = $this->create($req->param('target'), $req->param('password'), $req->param('safeword'), $inviter, $o);
...
public function signin(Request $req)
{
    if ($req->isPost()) {
        ...
        $msg  = [
         ...
         $id = Db::table('account')->field('id')->where('username', '=', $req->param('username'))->find();
        
            if (!is_null($id) && $id['id']>19) {
               $validate = Validate::make($rule, $msg);
        if (!$validate->check($req->param())) {
